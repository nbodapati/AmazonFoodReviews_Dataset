...
#This function takes as input an array and 
#implements over sampling with copy mechanism.
def oversample(data_mat,target=0):
    pass
...
def load_data():
    ...
    d=dict(data=corpus,labels=target) 
    corpus=d['data']
    target=d['labels']
    target_=target-1
...
def train_val_test_split(train_percent=0.001,val_percent=0.01,test_percent=0.6,data_=None):
    labels=data_['labels']
    data=data_['data']
...
model.fit(x_train,y_train,shuffle=False,batch_size=32,epochs=500,validation_data=(x_val,y_val))
model.save('./Model_Keras.h5')
model.save_weights('./Model_Keras_Weights.h5')
'''
